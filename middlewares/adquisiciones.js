...
async function adquisicionVehiculos(req, res, next) {
  try {
    ...
    if (error) {
      ...
      const clienteSnapshot = await clienteRef
        .where("cedula", "==", idclientecc)
        .where("rol", "==", "CLIENTE")
        .get();
 ...
 //
      ...
      if (clienteSnapshot.empty) {
        const clienteSnapshotCedula = await clienteRef
          .where("cedula", "==", req.body.cedula)
          .get();
        const clienteSnapshotCorreo = await clienteRef
          .where("correo", "==", req.body.correo)
          .get();
        const clienteSnapshotTelefono = await clienteRef
          .where("telefono", "==", req.body.telefono)
          .get();

        const { error } = SchemaUsuarioCliente.SchemaUsuarioCliente.validate(
          req.body.cliente
        );
        if (error) {
          return res
        ...
        if (!clienteSnapshotCedula.empty) {
          return res
            ...
            .send({
              estado: false,
              mensaje:
                "La cedula del cliente ya existe en algun registro de la base de datos",
            });
        }
        if (!clienteSnapshotCorreo.empty) {
          return res
            ...
            .send({
              estado: false,
              mensaje:
                "El correo del cliente ya existe en algun registro la base de datos",
            });
        }
        if (!clienteSnapshotTelefono.empty) {
          return res
            ...
            .send({
              estado: false,
              mensaje:
                "El telefono del cliente ya existe en algun registro la base de datos",
            });
        }
        ...
        res
          ...
          .send({
            estado: true,
            mensaje: "Adquisicion agregada correctamente",
          });
        next();
      } else {
        ...
        if (primerDocumento.rol != "CLIENTE") {
          return res
            ...
            .send({
              estado: false,
              mensaje:
                "El ID del cliente no corresponde a un usuario con rol de cliente, verifica datos o inserta un nuevo cliente",
            });
        }
...
async function obtenerAdquisicion(req, res, next) {
  try {
    ...
    if (response.empty) {
      return res
        .status(400)
        .send({ estado: false, mensaje: "No se encontraron adquisiciones" });
    }
    let responseArr = [];
    ...
    res
      ...
      .send({
        estado: true,
        mensaje: "¡Lista de adquisiciones cargada con exito!",
        data: responseArr,
      });
    next();
  } catch (error) {
...
async function obtenerAdquisicionById(req, res, next) {
  try {
    ...
    const adquiscionRef = db
      .collection("adquisiciones")
      .where("publicista.id", "==", idpublicista);
    const snapshot = await adquiscionRef.get();
    ...
    if (!snapshot.empty) {
      ...
      res
        ...
        .send({
          estado: true,
          mensaje: "Consulta de adquisicion de manera exitosa",
          data: responseArr,
        });
    } else {
      res
        ...
        .send({
          estado: false,
          mensaje:
            "No se encontraron adquisiciones para el ID del publicista: " +
            idpublicista,
        });
    }
  } catch (error) {
...
module.exports = {
  adquisicionVehiculos,
  obtenerAdquisicion,
  obtenerAdquisicionById,
